...
*/
		$name = $_POST['name'];
        $phone = $_POST['phone'];
		mail("lilccherkasova@yandex.ru", "Заказ обратного звонка с сайта: черкасова-юрист.рф", "Потенциальный клиент с именем ".$name." просит перезвонить Вас на номер ".$phone);
		//mail("vasilyev-r@mail.ru", "Заказ обратного звонка с сайта: черкасова-юрист.рф", "Потенциальный клиент с именем ".$name." просит перезвонить Вас на номер ".$phone);
        $_SESSION['win'] = 1;
