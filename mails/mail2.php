...
*/
		$name = $_POST['name'];
        $phone = $_POST['phone'];
		mail("lilccherkasova@yandex.ru", "Заявка с сайта: черкасова-юрист.рф", "Потенциальный клиент с именем ".$name." хочет получить консультацию и номер телефона: ".$phone);
		//mail("vasilyev-r@mail.ru", "Заявка с сайта: черкасова-юрист.рф", "Потенциальный клиент с именем ".$name." хочет получить консультацию и номер телефона: ".$phone);
        $_SESSION['win'] = 1;
